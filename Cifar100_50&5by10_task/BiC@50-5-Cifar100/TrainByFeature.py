...
def icl_training(task, last_classes, train_loader):
    ...
    if task >= 3:
        bias_list = Load_All_Bias(task,  base_classes, incre_classes, previous_epoch = 0)    
        bias_list.to(device)
        para_dict +=[{'params': bias_list.parameters(), "lr" : lr_dict["base"]}]
...
def BiC_training(task, train_loader):
    ...
    base_CNN = Load_Model(target_classes, task)     
    base_CNN.eval()
    base_CNN.to(device)    
    ...
    # decide the training parameter
    para_dict  = [  {'params': cor_bias.parameters(), "lr" : lr}]      
    ...
    for epoch in range(epochs):
        ...
        for i, (feature, label) in enumerate(train_loader): 
            ...
            pred = base_CNN(feature, "f")

            if task >= 3:
                pred = bias_list(pred)

            cor_pred = cor_bias(pred)
            ...
            # model pred result
            class_pred = torch.argmax(cor_pred, dim = 1).view(label.shape)           
            acctker.accumlate(torch.sum(class_pred == label).item(), label.shape[0])
            ltker.accumlate([loss.item()])                    
        ...
        if (epoch+1)== epochs:         
            ...
            # save model and ss set
            file = record_root+"//"+'task{}_{}.pth.tar'.format(task, cfg.bic_model_name)
            save_checkpoint(cor_bias.state_dict(), filename = file)
            ...
            path =  record_root+"//"+"task{}_{}_history.json".format(task, cfg.bic_model_train_his_name)
            SaveJson(path, his_dict)        
