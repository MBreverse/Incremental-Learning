...
class BiasLayer(nn.Module):
    def __init__(self):
        ...
        self.alpha = nn.Parameter(torch.ones(1, requires_grad= self.training))
        self.beta = nn.Parameter(torch.zeros(1, requires_grad= self.training))
...
class BiasList(nn.Module):
    def __init__(self, b_list, base_classes, increm_classes):
        super(BiasList, self).__init__()
        
        self.bias_list = nn.ModuleList(b_list)
        self.base_classes = base_classes
        self.icl_classes = increm_classes
        
    def forward(self, x):

        outputs = [x[:, 0:self.base_classes]]
        
        for tid, b in enumerate(self.bias_list):
            
            init_digits = tid * self.icl_classes  +  self.base_classes
            end_digits = (tid + 1) * self.icl_classes  +  self.base_classes 

            out = b(x[:,init_digits:end_digits])

            outputs.append(out)
        
        if x.shape[-1] - end_digits != 0 : # if bias process the end digit of tensor
            outputs.append(x[:, end_digits:x.shape[-1]])

        cat_out = torch.cat(outputs, 1)
