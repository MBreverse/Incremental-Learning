...
def Load_Bias(task, previous_epoch = 0):
    
    #load model
    bias = BiasLayer()
    
    if previous_epoch == 0:
        model_dict = torch.load(record_root + '/task{}_{}.pth.tar'.format(task, cfg.bic_model_name))
    else:
        model_dict = torch.load(record_root + '/task{}_{}_{}.pth.tar'.format(task, previous_epoch, cfg.bic_model_name))
    
    bias.load_state_dict(model_dict)
...
def Load_All_Bias(cur_task,  base_classes, increm_classes, previous_epoch = 0):
    
    #load model
    b_list = []
    for t in range(2, cur_task):
        
        b_list.append(Load_Bias(t))
    
    bias_list = BiasList(b_list,  base_classes, increm_classes)
