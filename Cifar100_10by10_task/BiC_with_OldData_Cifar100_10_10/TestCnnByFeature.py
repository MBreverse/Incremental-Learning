...
def test_bic_cnn_for_all_training_task( final_classes = 50, final_task = 1):
    ...
    for cur_task in range(1, final_task+1):
        ...
        if cur_task >= 2:
            bias_list = Load_All_Bias(cur_task+1,  base_classes, increm_classes, previous_epoch = 0)  
            bias_list.eval()
            bias_list.to(device)
