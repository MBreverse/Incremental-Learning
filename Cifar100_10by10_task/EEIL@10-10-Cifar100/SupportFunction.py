...
def Load_Model2(model_type, class_digits, task, root, name):# Create for NN-based
    
    model_type_dict = {"re-CNN": Recoverable_CNN}
    
    #load model
    model = model_type_dict[model_type](class_digits)
    
    model_dict = torch.load(root + '/task{}_{}.pth.tar'.format(task, name))
...
def Load_Kernel( insertion_check_list, kernel_sizes, task, training=False, previous_epoch = 0):
    ...
    if os.path.exists(kernel_path):
        ...
        kernel_list1 = Multi_Scale_Shift(insertion_check_list, kernel_sizes)
...
def module_reconstruction(module, origin_classes, target_classes, first = False):    
    ...
    Linear_dict = {"nn":nn.Linear,"Re":Recoverable_Linear }
    ...
    if first == False:
        ...
        f = Linear_dict["Re"](inp_size, target_classes) 
        f.weight.data[0:origin_classes] = last_weight
        f.bias.data[0:origin_classes] = last_bias          
        module = f
        ...
        f = Linear_dict["Re"](target_classes,out_size)        
        f.weight.data[:,0:origin_classes] = last_weight
