...
from ModelCollection import Resnet50_FeatureExtrator, CVAE
from SupportFunction import Load_Model2, one_hot
from Config import *
    ...
    def __init__(self, dataset , num_exemplar, mode = "rand", label_format = "origin"):
        ...
        for features, label in data_loader:
            ...
            for l in uni_label:
                ...
                if key not in class_feature_dict.keys():
                    idxs = torch.argwhere(label==l)[:,0]
                    class_feature_dict.update({key:[features[idxs,:]]})

                else:
                    idxs = torch.argwhere(label==l)[:,0]
                    class_feature_dict[key].append(features[idxs,:])        
        
        #Sample Data from data of each class
        m = num_exemplar
        print(m)
        print(class_feature_dict.keys())
        ...
        for key in class_feature_dict.keys():
            ...
            if label_format == "origin":
                self.stack_exemplar_label.append(torch.ones(class_feature_dict[key].shape[0])*float(key)) 
        ...
        print(self.stack_exemplar_feature.shape)
        print( self.stack_exemplar_label.shape)
...
class CVAE_PseudoExempar(Dataset):
    ...
    def __init__(self, sample_per_class, class_digits, task):
        ...
        self.output_dim = cur_class
        feature_size = cur_class
        model = CVAE(cur_class, cfg.z_dim, cur_class).to(device)  
    ...
    def __getitem__(self, index):
        ...
        return prototype.view(self.output_dim), target
...
class CNN_Encoding(Dataset):
    
    def __init__(self, task, loader):
        super(CNN_Encoding, self).__init__()
        
        total_digit = cfg.base_classes + (task-1) * cfg.incre_classes
        cnn = Load_Model2(model_type = "re-CNN", class_digits = total_digit , task = task, root = cfg.record_root, name = cfg.save_cnn_name)
        cnn.eval()
        cnn = cnn.to(device)
        ...
        for i, (x,y) in enumerate(loader):
            ...
            stack_encoding.append(encoding.detach())
            stack_label.append(y)
        
        self.encoding = torch.cat(stack_encoding, dim = 0).cpu()
        self.label = torch.cat(stack_label, dim = 0).cpu()
     
    def __getitem__(self, idx):
        
        return   self.encoding[idx],  self.label[idx]
