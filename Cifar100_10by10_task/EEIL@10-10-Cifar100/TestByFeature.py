...
def test_all_task_acc(base_classes = 50, increm_classes = 5, final_task =2):
    ...
    for cur_task in range(1, final_task+1):
        ...
        cnn = Load_Model2(model_type = "re-CNN", class_digits = cur_classes, task = cur_task, root = cfg.record_root, name = cfg.save_cnn_name)
        ss_path = cfg.record_root + '/task{}_{}.pth.tar'.format(cur_task, cfg.save_ss_name)        
        
        if os.path.exists(ss_path):            
            model = SS_Model3(cnn)
            model.build_SS()
            ss_state_dict = torch.load(ss_path)
        ...
        for test_task in range(1, cur_task+1):
            ...
            with torch.no_grad():
                ...
                for i, (feature, label) in enumerate(test_loader):
                    ...
                    # if cur_task == 2:
                        # output0 = cnn(feature, 'f')
                        # output = output + output0

                    final_pred = torch.argmax(output, 1)
...
def test_all_task(base_classes = 50, increm_classes = 5, final_task =2):
    ...
    for cur_task in range(1, final_task+1):
        ...
        model = SS_Model_2( cnn, insertion_check_list, kernel_size, all_kernel_list )
