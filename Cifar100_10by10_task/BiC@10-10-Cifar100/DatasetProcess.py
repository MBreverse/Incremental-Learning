...
class ImageNetFeatureSet(Dataset):
    
    def __init__(self, f_list, y_list):
        super().__init__()
        
        self.features = torch.cat(f_list, 0)
        self.labels = torch.cat(y_list, 0)
    
    def __getitem__(self, idx):
        
        f = self.features[idx].view(2048)
...
def Bulid_Merge_Class_Dataset(dset_list):
    
    train_set_list = [dset["train_true"] for dset in dset_list]
    valid_set_list = [dset["valid_true"] for dset in dset_list]
    
    train_task_set = torch.utils.data.ConcatDataset(train_set_list) 
    valid_task_set = torch.utils.data.ConcatDataset(valid_set_list) 
    
    TaskSet = {"train_true" : train_task_set,
               "val_true" : valid_task_set}
    
    return TaskSet

def Load_ImagetNet_Dataset(class_digit):
    
    data_root = "./data"
    data_name = "/ImageNet_Split"
    data_branch = "/feature_set"
    path = "{}{}{}/imagenet_split_{}_dataset.pth.tar".format(data_root, data_name, data_branch, class_digit)
...
def Save_ImagetNet_TaskSet(task_set, task_digit):

    data_root = "./data"
    data_name = "/ImageNet_Split"
    data_branch = "/task_feature_set"
    
    if not os.path.exists(data_root+data_name+data_branch):
        os.mkdir(data_root+data_name+data_branch)
    
    path = "{}{}{}/split{}_dataset.pth.tar".format(data_root, data_name, data_branch, task_digit)
    
    torch.save(task_set, path)

def Build_Classific_Dataset_of_Task():
    ...
    s_digit = 0
    tid = 0
    for classes in range(base_classes, final_classes+1, increm_classes):
        
        e_digit = classes
        
        tid += 1
        
        print(tid, s_digit,e_digit)

        dset_list = [] 
        for c in range(s_digit, e_digit):
            # print(c,end = " ")
            
            dset = Load_ImagetNet_Dataset(c)
            dset_list.append(dset)        
  
        task_set = Bulid_Merge_Class_Dataset(dset_list)
        Save_ImagetNet_TaskSet(task_set, tid)
        # sys.exit()
...
def repair_imagenet():
    ...
    for t in range(2,11):
        ...
        path = "{}{}{}/split{}_dataset.pth.tar".format(data_root, data_name, data_branch, t)
        dset = torch.load(path)  
        
        print(dset.keys())
        
        d = dset.pop("valid_true")
        print(dset, d)
        dset.update({"val_true":d})
        print(dset)
        # sys.exit()
        torch.save(dset, path)
...
def test_dataset_load():
    ...
    path = "{}{}{}/split{}_dataset.pth.tar".format(data_root, data_name, data_branch, 1)
    
    dset = torch.load(path)    
    print(dset)
    
    print(len(dset["train_true"]))    
    print(len(dset["val_true"]))
    
    # print(transforms.functional.to_pil_image())

if __name__ == "__main__":
    # Mnist_Split_Transform()
    Build_Classific_Dataset_of_Task()
    # repair_imagenet()
    # test_dataset_load()
