...
def Load_All_Task_SS(insertion_check_list, ss_sizes, cur_task, training = False):
    ...
    for i in range(2, cur_task + 1):
        ...
        ss_set = Load_Kernel( insertion_check_list, ss_sizes, i, training=False, previous_epoch = 0)
        
        all_ss_set_list.append(ss_set)
    
    return all_ss_set_list

def Load_AE(task = 1, previous_epoch = 0):
    ...
    if previous_epoch == 0:
        ...
        model_dict = torch.load('{}/task{}_{}_vae_model.pth.tar'.format(record_root, task, previous_epoch))
...
def get_Mnist_task_dataloder(data_root, branch, task , train, batch_size):
    
    dlabel = str(task)
    
    dataset_path = data_root + '/Mnist_split'+ branch +'//split'+ dlabel +'_dataset.pth.tar'    
    
    dataset = torch.load(dataset_path)       
    
    key = "train_true" if train == True else "val_true"
    shuffle = True if train == True else False
    
    dataloader = DataLoader(dataset[key], batch_size = batch_size, shuffle = shuffle) 
    
    return dataloader
    
def get_Cifar_task_dataloder(data_root, branch, task , train, batch_size):
