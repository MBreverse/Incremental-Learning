...
def base_training(dset, epochs = 10): 
    
    cfg = Config()
    num_exemplar = cfg.second_num_exemplar
    batch_size = cfg.batch_size
    base_classes = cfg.base_classes
    
    print("train CVAE:")
    # cvae_train_loader = DataLoader(dset, batch_size = batch_size, shuffle = True)     
    # train_base_CVAE(cvae_train_loader)
    
    print("train CNN:") 
    psedo_set = CVAE_PseudoExempar( sample_per_class = num_exemplar, class_digits = [0, base_classes-1] , task = 1 )    
    ConcateSet = torch.utils.data.ConcatDataset([psedo_set, dset])    
    
    cnn_train_loader = DataLoader(ConcateSet, batch_size = batch_size, shuffle = True)
    train_base_CNN(cnn_train_loader) 


def icl_training(task, dset, epochs = 10):
    ...
    print("train CVAE:")
    # cvae_train_loader = DataLoader(dset, batch_size = batch_size, shuffle = True)  
    # train_icl_CVAE(task, cvae_train_loader)
    ...
    for past_task in range(1, task):
        ...
        start_class_digit = (base_classes + (past_task - 2) * incre_classes)* (past_task > 1)
        end_class_digit = base_classes + (past_task - 1) * incre_classes - 1 
        AddPseudoSetList.append(CVAE_PseudoExempar( sample_per_class = num_exemplar, class_digits = [start_class_digit, end_class_digit] , task = task - 1 ))  

    prev_pseudo_set = torch.utils.data.ConcatDataset(AddPseudoSetList)

    start_class_digit = base_classes + (task - 2) * incre_classes 
    end_class_digit = base_classes + (task - 1) * incre_classes -1   
    pseudo_set = CVAE_PseudoExempar( sample_per_class = num_exemplar, class_digits = [start_class_digit, end_class_digit] , task = task )    
    
    # AddPseudoSetList = []
    # for past_task in range(1, task+1):
        
        # start_class_digit = (base_classes + (past_task - 2) * incre_classes)* (past_task > 1)
        # end_class_digit = base_classes + (past_task - 1) * incre_classes - 1     
        # print(start_class_digit, end_class_digit)
        # AddPseudoSetList.append(CVAE_PseudoExempar( sample_per_class = num_exemplar, class_digits = [start_class_digit, end_class_digit] , task = task ))  
    # assert False
    # curr_task_prev_pseudo_set = torch.utils.data.ConcatDataset(AddPseudoSetList)    
    
    ConcateSet = torch.utils.data.ConcatDataset([dset, pseudo_set, prev_pseudo_set])  
    
    cnn_train_loader = DataLoader(ConcateSet, batch_size = batch_size, shuffle = True)
    train_icl_CNN(task, cnn_train_loader)
